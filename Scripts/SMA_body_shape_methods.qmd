...
In a concurrent literature review (Skinner, in prep), I identify three predominant approaches used to quantify relative appendage length in the context of Allen’s rule:
...
Despite the near universal application of classical regression approaches in studies of Allen's rule (Skinner in prep), alternative approaches that allow for measurement error in $X$ may be more appropriate. For example, standardized major axis (SMA) regression EDITS HERE may be better suited for problems of biological scaling as they assume that measurement error is present in both $X$ and $Y$ [@wartonSmatr3Package2012]. The key difference in these types of regression is how they minimize residuals to generate a line of best fit. OLS regression minimizes the residuals on the $Y$ axis, whereas SMA regression minimizes the residuals in both $X$ and $Y$ directions (thus also called 'orthogonal regression'). This makes sense for situations where we don't think that $X$ -\> $Y$ (we wouldn't say that mass causes wing), but instead just want to understand the relationship between $X$ and $Y$ (CITE Smith, 2009). Thus, SMA regression can more accurately estimate the true functional relationship between two variables [@greenMassLengthResiduals2001; @wartonBivariateLinefittingMethods2006; @peigNewPerspectivesEstimating2009; @peigParadigmBodyCondition2010].

The statistical distinctions between OLS and SMA have received attention in several fields including statistics (CITE Smith 2009), evolutionary biology (CITE Hansen, Pelabon), ecology, genetics (CITE Ponzi), and even astrophysics (1996 paper), and thus will not be discussed further here. However, I see important parallels between the quantification of body condition and relative appendage length, and thus I turn to Peig & Green's Standardized Mass Index [SMI, @peigNewPerspectivesEstimating2009; @peigParadigmBodyCondition2010]. The SMI was created to estimate energy reserves in animals independent of structural body size, and has been used as an index of body condition in thousands of papers. In short, their approach used SMA regression to derive a scaling exponent and then standardized each individual's mass to a common body size, effectively removing allometric effects from the measure of body condition.

Inspired by Peig and Green's SMI [@peigNewPerspectivesEstimating2009; @peigParadigmBodyCondition2010], I develop and evaluate a related approach for quantifying relative appendage length. The Standardized Length Index (SLI), like the SMI, leverages SMA regression to isolate departures from allometric expectations. It scales individual appendage length to a common body size within a group (e.g., species or population), based on empirically derived or biologically meaningful scaling slopes. The SLI is similar to OLS residuals but has several key advantages. The SLI 1) maintains the units of the original variable 2) is grounded in allometric scaling theory (CITE Huxley), and thus accounts for the power law relationship (i.e., it implies a multiplicative instead of an additive error function), and 3) is comparable across populations within the same species [@peigNewPerspectivesEstimating2009; @peigParadigmBodyCondition2010].

In this paper, I use simulations to evaluate how well the SLI performs relative to the three classical regression approaches described above. I compare these methods in their ability to recover the true effect of a simulated temperature increase on relative appendage length, under a range of conditions reflecting realistic levels of measurement and transient fluctuation error, trait covariance, and allometric scaling relationships. While I recognize that several authors have modeled absolute appendage length as a function of temperature change (Skinner, *in prep*), I will not address that here as this approach is statistically very straightforward. Finally, while I use Allen's rule here, these analyses and takeaways can apply to many studies examining departures from bivariate EDIT.
...
As I said previously, the SMA line of best fit has a slope equal to the OLS slope divided by the correlation coefficient.
