...
All three approaches attempt to control for body size to isolate changes in relative appendage length. These approaches share another key assumption – that the independent variable $X$ (body size in this case) is "fixed", and any lack of fit in the regression between $X$ and $Y$ is due to a larger or smaller than expected $Y$ (appendage length in this case). In other words, ordinary least squares regression assigns all error in the model to the $Y$ variable (i.e., the minimization of vertical residuals). Hereafter, I term any statistical approach making this assumption about the distribution of error between between $X$ and $Y$ as 'classical regression'. Importantly, other common analytical techniques utilizing maximum likelihood or Bayesian approaches make the same assumption unless models are explicitly formulated to the contrary.
...
N <- 3000
df_morph_l <- pmap(Cols, \(...) gen_data(..., n = N, meas_error = 0, transient_error_mass = .1, transient_error_append = 0))
...
Second, I simulated allometric scaling slopes of `r b_avg_12`, and -0.22. This plot shows the estimated SMA slopes from each hypothetical species.
