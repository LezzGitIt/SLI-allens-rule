...
Here, I evaluate how well two new metrics of relative appendage length, generally referred to as the 'standardized length index (SLI)', are able to more accurately capture changes in relative appendage length due to a simulated temperature increase. The SMA residuals approach uses the residuals from an SMA regression of as an index of relative appendage length, whereas the SLI is based on a modified version of the standardized mass index [@peigNewPerspectivesEstimating2009; @peigParadigmBodyCondition2010], but instead measures how long an organisms appendage is relative to its body size. Hereafter, I will refer to these two approaches as 'SMA approaches', as both leverage SMA regression to generate metrics of relative appendage length. I compare the SMA approaches to the 3 approaches regularly found in the literature. I expect that the SMA approaches will perform better than approaches 1-3 above, as it is based in the principles of allometric scaling theory which is well-suited to answering questions regarding spatial and temporal applications of Allen's rule. While I recognize that several authors have modeled absolute appendage length as a function of temperature change (Skinner, *in prep*), I will not address that here as this approach is statistically very straightforward.
...
```{r}
#| label: vert-resid-fn
calc_vert_resid <- function(df, x, sma_mod){
  slope <- coef(sma_mod)['slope']
  intercept <- coef(sma_mod)['elevation']
  df2 <- df %>% mutate(pred = intercept + slope * {{ x }})
  df2 %>% mutate(vert_resid = {{ x }} - pred)
}
```
...
extract_coefs <- function(Sim_df) {
  ...
  Sim_df <- calc_vert_resid(df = Sim_df, x = Mass, sma_mod = sma_mod) 
  sma_resid <- lm(vert_resid ~ Temp_inc, data = Sim_df)
  ...
  tibble(
    ...
    coef_vert_resid_app = coef(sma_resid)["Temp_inc"], 
...
Parms_tbl3 <- Parms_tbl2 %>% 
  pivot_longer(
    cols = c(coef_vert_resid_app, coef_sli_iso_app, coef_sli_est_app, coef_ols_resid_app, coef_ryding_app, coef_ratio, coef_ratio2), 
