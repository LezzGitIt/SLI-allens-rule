...
where $\boldsymbol{\mu} = [5,\ 3,\ 3]^\top$ is the mean vector, and $\Sigma$ is a $3 \times 3$ covariance matrix constructed to yield biologically plausible correlations among variables. This simulation strategy avoids biasing results toward any method. For example, simulating from a directional model, e.g.,
...
ex_cov_mat <- gen_3var_cov(b_sma_12 = 0.33, r_12 =.5, r_13 = -.1, r_23 = -.4) 
round(ex_cov_mat$Sigma, 2)
...
Parms_mat2 <- Parms_mat %>% filter(!(r_13 == 0 & r_23 == 0))
```
...
Parms_mat2 <- bind_rows(Parms_mat2, Parms_inv) %>%
...
gen_ex_data <- function(Parms_mat){ #, transient_error = 0.04
...
#Parms_mat_pos <- Parms_mat2 %>% filter(Scaling != "Inverse") 
...
Ex_iso_prop <- Parms_mat2 %>% 
  filter(Temp_eff == "Proportional" & 
           Scaling == "Hyperallometry" & 
           r_12 == .45) %>% 
  slice_min(order_by = r_13) #with_ties = FALSE
# Generate datasets with and without transient error
Ex_no_te <- gen_ex_data(Ex_iso_prop, transient_error = 0)
Ex_te <- gen_ex_data(Ex_iso_prop, transient_error = .5)
