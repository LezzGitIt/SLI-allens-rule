...
While SMA regression can improve our inference in studies involving biological scaling [@wartonBivariateLinefittingMethods2006; @greenMassLengthResiduals2001], the assumptions of SMA regression can also be limiting. Importantly, when these assumptions are violated SMA regression also leads to biased estimates of allometric scaling coefficients [@smithUseMisuseReduced2009; @arnoldAllometricRelationshipSize2007; @wartonBivariateLinefittingMethods2006]. An alternative approach is to model true body size as a latent trait that is not directly observed, but estimated through the shared variance between mass (loading $\lambda_1$) and appendage length (loading $\lambda_2$), along with their respective error terms. Factor loadings are estimated from the data and indicate the extent to which each trait reflects the common latent factor (i.e., structural body size). This is conceptually similar to SMA regression in that both traits are treated symmetrically, and their shared variance is modeled without privileging one as the predictor. The latent factor represents structural size, and variance is partitioned between shared structure and trait-specific deviations. Temperature is included as a predictor of appendage length, while latent size is modeled through a factor structure shared with mass. This partitions appendage variation into components attributable to body size and components uniquely associated with temperature.

Let $i$ index individuals, such that the observed traits of individual $i$ are modeled as: $$
\text{Mass}_{i} = \lambda_1 \cdot \text{Size}_{i} + \epsilon_{i}
$$ {#eq-mass-latent-factor-model}

$$
\text{Appendage}_{i} = \lambda_2 \cdot \text{Size}_i + \delta_{i}
$$ {#eq-appendage-latent-factor-model}

-   $\text{Size}_i$ is a latent variable representing true structural size.
-   $\lambda_1$, $\lambda_2$ are factor loadings (scaling relationships).
-   $\epsilon_{i}$ and $\delta_i$ captures all variation in mass and appendage, respectively, that is not captured by latent $\text{Size}_i$

Thus we can use $\delta_i$ as our estimate of relative appendage length, which is then estimated as to test Allen's rule:

$$
\delta_i \sim \mathcal{N}(\beta_T \cdot \text{Temp}_i, \sigma^2)
$$ {#eq-latent-shape-model}

where $\beta_T$ is the estimand of interest and $\sigma^2$ represents residual variation in relative appendage length that is not explained by temperature. With repeated measurements at the appropriate time scale, additional hierarchical levels can be added to this model to further partition residual error. For example, consecutive measurements from different observers, devices, or field stations, but on the same subject, can be used to quantify measurement error (ME), while repeated measurements across a few days can help estimate transient fluctuations (TF) in mass (and appendage, if relevant). In this case, the latent factor model of mass (@eq-mass-latent-factor-model) transforms into $\text{Mass}_{ij} = \lambda_1 \cdot \text{Size}_i + \epsilon^{ME}_{ij} + \epsilon^{TF}_{ij} + \epsilon_{ij}$ where $Mass_{ij}$ is the mass of individual $i$ on measurement $j$ and $\epsilon_{ij}$ soaks up any additional variation in mass not explained by latent size or repeat measurements (i.e., ME and TF). Partitioning extraneous variance in mass and appendage can help achieive better estimates of latent size and ultimately $\delta_i$, our index of relative appendage length. If unaccounted for, these sources of error are absorbed into the residual variance of the shape model (@eq-latent-shape-model), attenuating the estimated effect of temperature on relative appendage length ($\beta_T$).

This latent variable structural equation model has not been used (at least in recent years, Skinner in prep) in the context of Allen's rule, but provides several key advantages. The model 1) accounts for allometric scaling without directional causality (i.e. that mass causes appendage), 2) partitions error and trait-specific noise and residual error, and, most importantly 3) provides clean inference on temperature's effect on relative appendage length. When estimated in a Bayesian framework, this class of models jointly estimates all parameters and naturally propagates uncertainty through all model levels. The framework is also easily extended to incorporate additional biological complexity, such as phylogenetic structure, random effects, or spatial and temporal autocorrelation.
