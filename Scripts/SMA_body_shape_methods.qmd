...
$$
\log(\text{Appendage}) \sim \alpha + \beta_1 * \log(\text{Body size}) + \beta_2 * \log(\text{Temperature increase}) + \epsilon
$$ {#eq-biased}
...
One important feature of SMA regression is that the $X$ and $Y$ axes can be flipped and the line of best fit will not change. In the plot below we flip the axes, & plot both the OLS (red) & SMA (blue) regression lines of best fit, as well as an individual from our hypothetical species (highlighted in green). Using the SMA (blue) regression line, you can see that the bird would be described as a relatively short-winged bird for its size in plot (A), or a relatively heavy bird for its wing length in plot (B). These are inverse statements, which make sense for any individual. Comparing the green point to the OLS (red) line, on the other hand, we see that these logical & reciprocal statements do not apply.
...
extract_coefs <- function(Sim_df) {
  ...
  # SLI
  Sim_df <- Sim_df %>% calc_sli(b_sli = 0.33)
  sli_app <- lm(sli_isometry ~ Temp_inc, data = Sim_df)
  ...
  tibble(
    ...
    coef_sli_app = coef(sli_app)["Temp_inc"],
...
Parms_tbl3 <- Parms_tbl2 %>% 
  pivot_longer(
    cols = c(coef_sma_resid_app, coef_sli_app, coef_ols_resid_app, coef_ryding_app, coef_ratio, coef_ratio2), 
...
The 5 approaches (2 SMA approaches, 3 traditional approaches) are all interested in accurately estimating $\beta_1$ from @eq-beta1-5-approaches.

$$
Wingyness \sim \alpha + \beta_1 * \mathit{Temperature\ increase}
...
Sli_fatter_wrong <- 100 - pull_percent("Sli", "Fatter")
Sma.res_much.wingyier_wrong <- 100 - pull_percent("Sma_resid", "Much wingyier")
...
Sli_proportional_pos <- pull_percent_proportional("SLI", "pos")
