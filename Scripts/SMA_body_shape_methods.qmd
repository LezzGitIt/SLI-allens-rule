...
The difference in the 5 approaches (note that Wing / mass and Wing² / mass are both 'ratio approaches') is how relative appendage length is measured (as outlined in the introduction). Note that the response variable would be the absolute appendage length in the 'multivariate regression' approach, and the model would also contain $\beta_S * \mathit{Body\ size}$ in @eq-betaT-5-approaches.
...
#| fig-cap: The true simulated effect above each plot, and the impact of temperature increase on relative appendage length on the y-axis. A positive slope means that birds get wingyer as temperature increases, and a negative slope means that birds get fatter as temperature increases. Each point represents a hypothetical dataset from a hypothetical species. Points are colored by the strength of the true effect in the simulated species, where lighter shades of blue mean a given increase in temperature results in a proportionally fatter (left panel) or wingyier (right panel) bird. The bird relative appendage length figure was borrowed from Youngflesh et al (2022), but will be changed before submission of the manuscript. 
#| fig-width: 11
#| fig-height: 8
#| fig-pos: 'H' 
...
Ex_parms_prop <- Parms_mat2 %>% 
  filter(Scaling %in% c("Isometry", "Hypoallometry") & Temp_eff == "Fatter" & r_13 == -.4 & r_12 == .45) %>% 
...
## Positive bias in the SLI approach

Despite its advantages, the SLI isometry method is not without flaws. In particular, we observed positive bias in cases involving hypoallometric scaling (true slope \< isometry). As illustrated in Figure 10, this occurs because the SLI isometry approach uses a steeper line (the isometric slope) to standardize appendage length. For small individuals with short wings (e.g., bottom left of the plot), the projection onto the isometric line inflates their relative appendage scores, especially when they have experienced large temperature increases. As a result, these individuals are erroneously classified as wingy, even when the species is actually getting fatter. This illustrates that when the assumed isometric slope differs substantially from the species' true allometric relationship, standardization can mislead.

The positive bias observed in the SLI isometry approach can be attributed to a mismatch between the assumed isometric scaling (orange line) and the true, hypoallometric relationship between mass and wing length (blue line). In these cases, the true allometric slope is shallower than the isometric slope of 0.33. When individuals from a hypoallometric species experience higher temperatures and become smaller, their absolute appendage length may remain relatively unchanged or even decrease slightly. However, when SLI isometry standardizes these individuals to a common body size using the isometric slope, it effectively projects them onto a steeper line (the orange line in @fig-sli-pos-bias). This makes the individuals appear to have relatively longer appendages than they truly do, especially those in the bottom-left of the plot that experienced the greatest temperature increases. As a result, the SLI isometry metric assigns these individuals high values of relative appendage length, despite the species actually getting fatter, leading to positively biased estimates of $\beta_T$ in these scenarios.
