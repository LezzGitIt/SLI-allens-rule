...
Let $i$ index individuals, such that the mass and length of individual $i$ are modeled as: $$
...
-   $\epsilon_{i}$ and $\delta_{i}$ captures all variation in latent mass and length, respectively, that is not captured by latent $\text{Size}$. Importantly, $\delta_{i}$ is our measure of relative lengthiness, i.e. length adjust for body size.
...
$$
\text{Mass}_{ij} = \lambda_{mass} \cdot \text{Size}_i + \epsilon^{TF}_{ij} + \epsilon_{ij} 
...
$$
\delta_{i} \sim \mathcal{N}(\beta_T \cdot \text{Temp}_i, \sigma^2)
