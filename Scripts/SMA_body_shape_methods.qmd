...
r_12 <- c(.3, .45, .6) #c(.01, .05, .1) # wing mass
...
A literature review of studies since 2021 reveals three predominant approaches used to quantify relative appendage length in the context of Allen’s rule (@fig-lit-review), where $\beta_T$ is the estimand of interest in all three cases.
...
Lr <- Lit_review %>%
  ...
  dplyr::select(
    authors, year, regression_used, class, starts_with("approach")
    ) %>%
...
Lr_pivot <- Lr %>%
  ...
  filter(approach_unclear == "No")  # exclude studies with unclear methodology
...
Lr_p2 <- Lr_p %>%
  group_by(class) %>%
  mutate(approach_facet = fct_reorder(paste(class, approach_label, sep = "___"), num_studies, .desc = TRUE),
         Per = paste0(Per, "%")) %>%
  ungroup()
...
pull_per_aves <- function(approach_name){
  result <- Lr_p %>% filter(class == "Aves" & str_detect(approach, approach_name)) %>% pull(Per)
...
I conducted a literature review of studies since 2021 examining the statistical and methodological approaches employed to test for Allen's rule [following @rydingShapeshiftingChangingAnimal2021]. I searched for papers using the keywords Allen\* AND relative appendage OR appendage OR limb OR body shape AND climate change OR warming OR temperature OR thermoregulation OR latitude, and also examined references in retrieved articles for additional papers. To be included, articles had to 1) be published between 2021–2025, 2) examine Allen's rule across environmental (temperature, productivity, etc.) or geographic (e.g. latitude) gradients, and 3) have endothermic focal species. In total, I scored `r nrow(Lit_review)` papers meeting the criteria, of which `r nrow(Unclear_exclude)` were excluded as they did not meet the criteria. The majority of studies examined Allen's rule in birds (`r per_aves`), while mammals were the focal taxa in the remaining studies. I identified three primary approaches used to understand how *relative* appendage length varies with respect to geography or climate: 'body size as a covariate' in multivariate regression (`r per_mv_reg`% of bird studies), two-step residual regression (`r per_residual`%), and ratios (`r per_ratios`%; @fig-lit-review). A small number of studies used absolute instead of relative appendage length (`r per_raw`%), and a handful were classified as 'Other' (including structural equation modeling, phylogenetic meta-analysis, and a rotation-matrix approach).

```{r}
#| label: fig-lit-review
#| fig-cap: Prevalence of the primary techniques used in the literature to test Allen's rule. Each point represents a study, and studies that implemented multiple approaches are connected via colored lines to show non-independence.
#| fig-height: 7
#| include: true
...
ggplot() +
  ...
  geom_line(
    data = Lr_stack2,
    aes(x = approach_facet, y = ypos, group = study, color = study), alpha = .75
  ) +
  geom_text(
    data = Lr_p2, aes(x = approach_facet, y = num_studies, label = Per),
    vjust = -0.7, size = 5
  ) +
  facet_wrap(~class,
    scales = "free_x",
  ...
  theme(
    axis.text.x = element_text(size = 10, vjust = .58, angle = 60),
    legend.position = "top"
  ) +
  guides(color = "none") +
...
\end{bmatrix}
\sim \mathrm{MVN}(\boldsymbol{\mu}, \Sigma)$$ {#eq-mvn}
...
Parms_mat2 <- bind_rows(Parms_mat, Parms_inv) %>%
  mutate(
    Temp_eff = factor(
      Temp_eff, levels = c("Fatter",  "Proportional", "Longer", "Much longer")
      ), 
    Scaling = factor(
      Scaling, levels = c("Hypoallometry", "Isometry", "Hyperallometry", "Inverse"))
...
As mentioned in the introduction, estimates of body size and appendage length are always influenced by measurement error, and some measurements are subject to additional types of error. In particular, when mass is used as the proxy for body size, transient fluctuation 'error' should not be ignored given that it is likely substantially larger than measurement error. Thus, I include an equivalent amount of measurement error in both $X$ and $Y$ (2% of the standard deviation of $X$ and $Y$), as well as transient fluctuation 'error' in mass equal to 10% of the standard deviation of $X$.
...
#| fig-cap: Ensure the allometric scaling slopes set during simulation were recovered. SMA (triangles) and OLS (circles) slopes were set so that their average was equivalent to the specified scaling slope (0.22, 0.33, 0.44, or -0.22).
#| fig-pos: 'H'
...
$$SLI \sim \mathit{Temperature\ increase}$$ {#eq-SLI-temp-mod}
...
I observed the same rank-order pattern in nightjars as in our simulation study: the ‘multiple regression’ and 'two step residual regression' approaches consistently yielded more negative estimates than the SLI methods, while the ratio approaches produced more positive estimates across all three species. Biological conclusions vary greatly depending on the approach used – all three species are getting relatively longer according to the wing / mass ratio, whereas all three species are getting relatively fatter if using the multiple regression or OLS residuals approaches. In contrast, the SLI isometry approach, which was least biased in my simulations, implies divergent responses: nighthawks get wingyer (although the 95% barely overlaps 0) with increasing temperature, whereas European nightjars and whip-poor-wills get relatively fatter. Importantly, the Wing/Mass ratio was strongly negatively correlated with mass in all three species ($r < -0.9$), which likely drives its positive bias as body size declines with increasing temperatures (i.e., Bergmann’s rule). Squaring wing length before dividing by mass (Wing²/Mass) reduced this correlation ($-0.81 < r < -0.66$), suggesting it may be a more effective, albeit still imperfect, proxy for shape.

## Case study: Atlantic forest birds

![Patterns observed during simulation were also seen in six species (N between 66 and 299 individuals) of Atlantic forest birds. The panels are organized by a species adherence to Bergmann's rule, such that in a) neither trait followed Bergmannian patterns, in b) species exhibited inverse Bergmann's rule (larger with increasing temperatures), in c) at least one trait decreased with temperature (in accordance to Bergmann's rule), and in d) traits exhibited mixed responses to Bergmann's rule, such that one trait increased in size while the other decreased in size. The shape of the points indicates which trait changed across temperature gradients (both mass and wing, just mass, or neither).](../Figures/Atlantic_birds_shape.png){#fig-Atlantic-birds width="100%"}

Results from Atlantic forest birds of Brazil are generally similar (but less pronounced) compared to migratory nightjars in North America and Europe. This is likely because the correlation between wing and mass was weak or non-existent in most species, highlighting an important limitation of the SLI method: that SMA regression should only be used when there is a significant relationship between the morphological variables of interest. Indeed, SMA regression has Joliceur 1990 argues that the correlation should be greater than 0.6, which is extremely high. Furthermore, the relationship between temperature and morphology is generally weak (panel a in @fig-Atlantic-birds), and inconsistent (panels b and d in @fig-Atlantic-birds). Thus, temperature may have little effect on size and shape in Atlantic forest birds.

That being said, the patterns were largely the same as those observed in simulations and in our case study of nightjars. The ratio approaches estimated temperature's effect on relative appendage length as most positive, the 'mass as a covariate' / OLS residuals approaches were most negative, and the SLI approaches were in the middle. Importantly, this pattern was reversed in species exhibiting inverse Bergmann's rule (panel b in @fig-Atlantic-birds). This makes sense as increasing temperatures would push individuals up into the top right quadrant of @fig-allometric-relationship-temp-ral. Thus, classical regression approaches suggest that the individuals in Section II have relatively long appendages (i.e., positive residuals), whereas SMA labels them as being relatively fat.

# Discussion

In simulations the 'SLI isometry' approach appears to be the most flexible and generalizable of all the approaches I examined for quantifying relative appendage length, though it does exhibit negative bias under biologically realistic scenarios. The multiple regression approach returned negatively biased estimates of temperature's impact on relative appendage length, whereas the ratio approach appeared positively biased. The different approaches showed nearly identical rank-order patterns in estimates of relative wingyness using empirical data of three migratory nightjars. Importantly, the method used had a major impact on the biological conclusions drawn regarding temperature-relative appendage length gradients in both simulations and the nightjar case study. Thus, I recommend a reexamination of past data sets with a more critical eye towards the methodologies utilized, and interpretation of results with the biases of each methodology in mind.

## Multiple regression and absolute appendage length

There are two main reasons SLI approaches outperform classical regression approaches in this context. The first is specific to the multiple regression approach, which has been advocated for by authors in recent years [e.g., @rydingResponseAllometryEvaluate2022; @rydingMultipleRegressionNot2025], and was the most commonly utilized approach to quantify relative appendage length in a concurrent literature review (Skinner et al in prep). Users of the 'multiple regression' approach often interpret $\beta_T$ (in @eq-betaT-5-approaches) as the effect of temperature on appendage length relative to body size. However, the response variable in @eq-betaT-5-approaches is still the *absolute* appendage length. In essence, the multiple regression model asks the question 'At each fixed mass, how does temperature influence absolute appendage length?' This is distinct from the question that often motivates the study: ‘How does temperature influence appendage length relative to body size?’.
...
#| fig-cap: In species simulated to get relatively longer, the negative bias is greatest at more negative temperature-appendage correlations. The color of points depicts the difference between temperature's effect on wing and its effect on mass. The size of points shows the correlation between appendage and mass, highlighting that negative bias is more extreme when the correlation between mass and appendage is low.
...
df_labs <- Ex_df_neg2 %>%
  ...
  mutate(
    lab = Mass_bin %>%
      ...
      gsub("\\(|\\]|\\[", "", .) %>%
      strsplit(",") %>%
      sapply(function(x) paste("log(Mass) =", x[1], "-", x[2]))
  )

# map labels vector
lab_map <- setNames(df_labs$lab, df_labs$Mass_bin)
...
#ggsave("Figures/Mass_as_collider.png", bg = "white")
```

```{r}
#| label: reg-mod-mass-collider
#| include: false
...
plot_allo_temp <- function(df, alpha = .7, lm_line = TRUE, label = NULL, ...){
  ...
  if(!is.null(label)){
    ...
    p <- p + annotate("text", x = 4.23, y = 5.13, label = "I", size = 6, color = "red") +
  annotate("text", x = 4.52, y = 5.25, label = "II", size = 6, color = "red")
  }
...
Classical regression always underestimates the functional relationship between mass and appendage (as long as $r$ \< 1). Relative to SMA, this leads to an overestimation of negative residuals in small individuals (i.e., points in the bottom left quadrant of the plot - labeled with a "I"), and an overestimation of positive residuals in large individuals (top right quadrant of the plot - labeled with a "II"). If we assume that the temporal analog of Bergmann's rule applies to most species, then temperature increases will push individuals towards the bottom left quadrant of the plot – exactly where classical regression overestimates the negative residuals. In other words, classical regression approaches suggest that the individuals in Section I are relatively fat (i.e., negative residuals), whereas SMA labels them as having relatively long appendages.
...
Ex_parms_prop <- Parms_mat3 %>% 
  filter(Scaling %in% c("Isometry", "Hyperallometry") & Temp_eff == "Longer" & r_12 == r_12[1] & r_23 == -.7) %>% 
...
model <- '
  ...
  # latent measurement model for mass & length
  Length =~ Appendage_1 + Appendage_2 + Appendage_3
  Mass =~ M1 + M2 + M3
...
Let $i$ index individuals, such that the mass and length of individual $i$ are modeled as: $$\text{Mass}_{i} = \lambda_{mass} \cdot \text{Size}_{i} + \epsilon_{i}$$ {#eq-mass-latent-factor-model}

$$\text{Length}_{i} = \lambda_{length} \cdot \text{Size}_{i} + \delta_{i}$$ {#eq-appendage-latent-factor-model}
...
- $\epsilon_{i}$ and $\delta_{i}$ captures all variation in latent mass and length, respectively, that is not captured by latent $\text{Size}$. Importantly, $\delta_{i}$ is our measure of relative lengthiness, i.e. length adjusted for body size.

#### Measurement model

In this model, mass and length (or “lengthiness”) are also latent variables, with each individual’s true mass and lengthiness reflected in the shared variance among their manifest indicators (M1–M3 and A1–A3, respectively). Paramaterizing length as latent is a parsimonious way to use information from any number of appendage variables (e.g., ear, hind foot, total body length). Paramaterizing mass as latent accounts for imperfect measurement due to transient fluctuations or true measurement error (assuming repeat measurements at an appropriate time scale). Note that any appendage variable with repeat measurements could also be parameterized as latent, which could be important in small species or traits with small dimensions [e.g., bill width; @theriotHarnessingNaturalHistory2023].
...
where $Mass_{ij}$ is the mass of individual $i$ on measurement $j$, $\epsilon^{TF}_{ij}$ captures transient fluctuations, and $\epsilon_{ij}$ soaks up any additional variation in mass not explained by latent size or repeat measurements. Partitioning residual variance in mass can improves estimates of latent size and ultimately ultimately $\delta_{i}$ (@eq-appendage-latent-factor-model), our index of relative lengthiness.

#### Structural model

The structural model is defined by our question of interest - temperature's effect on relative length (Allen's rule), such that $\delta_{i}$ is estimated as:
...
- Cite Nakagawa 2017 for problems with multiple regression approach (which they call the ‘intermediate outcome’ problem; also cite Gelman, 2014 & others). Admittedly I don't fully get the issue, but they say if scenario 2a is true 'then correcting for x leads to an underestimation of the direct treatment effect (b1 in Eq. 5) on y (known as over-adjustment bias)'. This seems like the situation we have here, where temp has a direct effect on mass and appendage, and mass and appendage are correlated as a result.

- We don't really know the true underlying causal process between mass and wing. In some species mass may drive wing, in others wing may drive mass. Overall, a correlated model probably makes the most sense. The true underlying causal process will influence the correct way to handle the problem statistically & obtain an unbiased estimate. Remember that in our simulation I just simulated correlated values, but no causal process. See McElreath page XX.

- Ryding et al (2025) does a good job making the argument why heat dissipation through bird wings is important

  - See Lewden (2023)

  - Note inert wing feathers are highly correlated with underlying bone length (Subasinghe et al. 2021, Weeks et al. 2025)

  - They don't explain the increase in heat production during flight, which seems like important piece of the puzzle

- Symonds simulates absolute vs relative appendage length models. Ryding & McQueen compare approaches to modeling temperatures effect on relative appendage length

- The treatment -\> fungus -\> height situation (McElreath) is known as 'directional separation', or post-treatment bias. This would be like if temperature is separated from appendage when we condition on mass.
...
```{r}
# TEST
test <- pmap(Cols[1,], \(...) gen_data(..., n = N, meas_error = 0, transient_error_mass = 0, transient_error_append = 0)) 
ggplot(data = test[[1]], aes(x = Mass, y = Append)) +
  geom_point() + 
  geom_smooth()
```
