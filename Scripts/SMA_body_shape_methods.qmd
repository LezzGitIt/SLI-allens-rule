...
#| fig-cap: The prevalence of 4 primary techniques used in the literature to test Allen's rule. 
...
To evaluate the methods on a large range of data sets, we also simulate some species that exhibit negative allometry [as observed in @weeksSharedMorphologicalConsequences2020], where as mass decreases appendage length actually increases (advarserial data sets; [@lotterhosSimulationTestsMethods2022].
...
## SALI: Standardized appendage length index

Peig & Green [-@peigNewPerspectivesEstimating2009]; [-@peigParadigmBodyCondition2010] revolutionized the way we estimate body condition, using allometric scaling theory to create the standardized mass index (see 3 step process outlined on pages 1886 & 1887 of [@peigNewPerspectivesEstimating2009]). We employ a similar process but instead create the standardized appendage length index (SALI). This is very similar to the SMA residuals approach, except that you set the slope $B_{SALI}$ to a value with biological significance. For example, setting $B_{SALI} = 0.33$ compares the appendage length of individual $i$ to a hypothetical species exhibiting isometric scaling with the same body size as individual $i$.

To calculate the SALI for individual $i$ of species $j$, first take the mean of the body size trait in question (e.g., mass) of species $j$, which serves to standardize against the appendage length of individual $i$. We call this $L_0$ as in Peig & Green [-@peigNewPerspectivesEstimating2009]; [-@peigParadigmBodyCondition2010].\
\
$L_0 = mean(log(\mathit{Body\ size}))$

$SALI_i = \log(\text{Appendage}_i) * \dfrac{L_0}{\log(\text{Body size}_i)^{\beta_{SALI}}}$

Finally, we would model the SALI as a function of temperature increase.

$SALI \sim \mathit{Temperature\ increase}$

Note that setting $B_{SALI} = 0.33$ is similar to the approach taken by Youngflesh et al. [-@youngfleshAbioticConditionsShape2022] in creation of their 'wingyness' index, as they explicitly wanted to capture departures from isometry.

```{r}
#| label: calc-sali-fun

calc_sali <- function(df, b_sali = 0.33){
  # L0 is the average mass, essentially allowing for comparison of wing lengths for a given mass
  L0 <- mean(df$Mass)
  df %>% mutate(sali = Wing * (L0 / Mass)^b_sali) %>%
  arrange(desc(sali))
}
```
...
extract_coefs <- function(Sim_df) {
  ...
  # SALI
  Sim_df <- Sim_df %>% calc_sali()
  sali_app <- lm(sali ~ Temp_inc, data = Sim_df)
  ...
  tibble(
    ...
    coef_sali_app = coef(sali_app)["Temp_inc"],
...
Parms_tbl3 <- Parms_tbl2 %>% 
  pivot_longer(
    cols = c(coef_sma_resid_app, coef_sali_app, coef_ols_resid_app, coef_ryding_app, coef_ratio, coef_ratio2), 
...
sali_fatter_wrong <- 100 - pull_percent("Sali", "Fatter")
...
Sali_proportional_pos <- pull_percent_proportional("SALI", "pos")
# Difference between SALI classification & reality 
Dif_sali <- Sali_proportional_pos - (Prop_wingyer * 100)

Dif_ryding <- pull_percent_proportional("Mass as covariate", "neg") - (Prop_fatter * 100)
```

1.  The SALI approach correctly estimates the direction of the simulated effect when temperature increases relative appendage length (i.e., a positive $\beta_1$ in @fig-compare-approaches; 'Wingyier' and 'Much wingyier' in the @fig-eval), but incorrectly estimates the direction of the simulated effect in about `r sali_fatter_wrong`% of hypothetical species when temperature decreases relative appendage length (i.e., 'fatter' species). In species that were simulated as decreasing in size proportionally, the SALI approach misclassifies about `r Dif_sali`% of hypothetical species as getting Wingyier that in reality decreased in relative appendage length (@fig-eval panel b). That being said, it correctly classified all species exhibiting negative allometry. Thus, the SALI approach appears flexible and generalizeable, but positively biased in situations that are likely common in the real world (e.g., that species decrease in size proportionally with climate warming).

    1.  The 'SMA residuals' approach correctly estimates the direction of the simulated effect when the species exhibit positive allometry 100% of the time, but misidentifies the relationship `r Sma.res_much.wingyier_wrong`% of the time when species exhibit negative allometry ('Much wingyier' in the @fig-eval). The SMA residuals approach appears to be unbiased when there is no effect of temperature on body shape, as it estimates a positive effect in about 50% of the simulated hypothetical species (@fig-eval panel b). Thus, the 'SMA residuals' approach appears to provide the best indicator of wingyness in species that exhibit positive allometry, which is very common in biology [@shingletonAllometryStudyBiological2010; @youngfleshAbioticConditionsShape2022].
...
plot_allo_temp <- function(df, label = NULL, ...){
  ...
  annotate("text", x = 6.5, y = 3.8, label = "II", size = 6, color = "black")
  }
...
#| fig-cap: Hypothetical species exhibiting negative allometry. The SMA residual approach does not capture the relationship between an increase in temperature and relative appendage length. 
